...
public class UDPServer {
	...
	public static void main(String[] args){
		...
		try{
		  ...
		  // 6786 포트를 가지는 DatagramSocket 객체를 생성.
		  // client 에게 DatagramPacket 를 수신 및 송신하기 위해
		  System.out.println("UDP Server ready");		 
		  while(true){
			  byte[] receiveFrom_c = new byte[512]; 
			  // UDP의 실제 데이터는 일반적으로 512바이트로 제한하는 경우가 많기 때문에 바이트 배열의 크기 512
			  DatagramPacket dp = new DatagramPacket(receiveFrom_c, receiveFrom_c.length); 
			  // DatagramPacket을 바이트배열 receiveFrom_c의 길이만큼 저장
