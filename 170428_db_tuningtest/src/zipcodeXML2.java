...
public class zipcodeXML2 {
	public static void main(String args[]){
		try{
			...
			ResultSet rest = stmt.executeQuery("select * from zipcode where rownum<5 order by seq");
			...
			tf.setOutputProperty(OutputKeys.DOCTYPE_SYSTEM, "c:\\XML\\ZCdtd.dtd");
			tf.setOutputProperty(OutputKeys.INDENT, "yse");
			DOMSource source = new DOMSource(doc);
			StreamResult result = new StreamResult(new FileOutputStream(new File("c:\\XML\\zipcode_2.xml")));
