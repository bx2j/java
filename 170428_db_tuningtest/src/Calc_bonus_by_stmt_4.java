...
public class Calc_bonus_by_stmt_4 {
	public static void main(String[] args) throws IOException, ClassNotFoundException,Exception {		
		...
		//String str = "select * from emp_large where job != 'PRESIDENT'"; // 직업이 PRESIDENT인 직원은 제외
		...
		try{
			while(rest.next()){
				...
				/*String sql = "insert into bonus_large3 values('" + 
						time1 + "'," + rest.getInt(1) + ",'" + rest.getString(3) + "'," + 
						rest.getInt(8) + "," + rest.getInt(6) + "," + bonus + ")";*/
				// 성능 저하의 원인??
				/*stmt1.executeUpdate("insert into bonus_large4 values('" + 
						time1 + "'," + rest.getInt(1) + ",'" + rest.getString(3) + "'," + 
						rest.getInt(8) + "," + rest.getInt(6) + "," + bonus + ")");*/
